...
test('Manifest Save and Restore - 1 App', async t => {
    let y: () => void;
    let n: (e: string) => void;
    ...
    app1 = await fin.Application.createFromManifest('http://localhost:1337/test/registeredApp1.json');
...
test('Manifest Save and Restore - 1 App 1 Child', async t => {
    let y: () => void;
    let n: (e: string) => void;
    const p = new Promise((res, rej) => {
        y = res;
    ...
    await saveRestoreCreateChildWindow(app1.me.uuid);
...
test('Manifest Save and Restore - 2 Snapped Apps', async t => {
    let y: () => void;
    let n: (e: string) => void;
    const p = new Promise((res, rej) => {
        y = res;
    ...
    app1 = await fin.Application.createFromManifest('http://localhost:1337/test/registeredApp1.json');
    app2 = await fin.Application.createFromManifest('http://localhost:1337/test/registeredApp2.json');
...
test('Manifest Save and Restore - 2 Tabbed Apps', async t => {
    let y: () => void;
    let n: (e: string) => void;
    const p = new Promise((res, rej) => {
        y = res;
...
test('Manifest Save and Restore - Deregistered - 1 App', async t => {
    let y: () => void;
    let n: (e: string) => void;
    const p = new Promise((res, rej) => {
        y = res;
    ...
    app1 = await fin.Application.createFromManifest('http://localhost:1337/test/deregisteredApp1.json');
...
test('Manifest Save and Restore - Deregistered - 1 App 1 Child', async t => {
    let y: () => void;
    let n: (e: string) => void;
    const p = new Promise((res, rej) => {
        y = res;
    ...
    const failIfWindowCreated = async (event: { topic: string, type: string, uuid: string, name: string }) => {
        if (event.name === 'Child-1 - win0' || event.name === app1.identity.uuid) {
            y();
            t.fail();
        }
    };
    ...
    await saveRestoreCreateChildWindow(app1.identity.uuid);
...
test('Manifest Save and Restore - Deregistered - 2 Snapped Apps', async t => {
    let y: () => void;
    let n: (e: string) => void;
    const p = new Promise((res, rej) => {
        y = res;
    ...
    app1 = await fin.Application.createFromManifest('http://localhost:1337/test/deregisteredApp1.json');
    app2 = await fin.Application.createFromManifest('http://localhost:1337/test/deregisteredApp2.json');


    const failIfWindowCreated = async (event: { topic: string, type: string, uuid: string, name: string }) => {
        if (event.name === `Child-1 - win0` || event.name === app2.identity.uuid) {
            y();
            t.fail();
        }
    };
    ...
    await saveRestoreCreateChildWindow(app2.identity.uuid);
...
test('Manifest Save and Restore - Deregistered - 2 Tabbed Apps', async t => {
    let y: () => void;
    let n: (e: string) => void;
    const p = new Promise((res, rej) => {
        y = res;
