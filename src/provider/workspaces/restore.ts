...
const createWorkspacePlaceholders = async(workspace: Workspace): Promise<void> => {
    ...
    async function createWorkspaceAppPlaceholders(app: WorkspaceApp) {
        ...
        if (isRunning) {
            ...
            // This function needs to be before the main window sets a new snap group, otherwise child windows may re-attach (if removeTab is called on an
            // application that it's tabbed to, leaving the child window as a remainingTab).
