...
export const restoreWorkspace = async(payload: Workspace, identity: Identity): Promise<Workspace> => {
    ...
    async function createAllPlaceholders(app: WorkspaceApp) {
        ...
        if (isRunning) {
            ...
            // These functions need to be in this order, otherwise child windows may re-attach (if removeTab is called on an application that it's tabbed to, leaving the child window as a remainingTab).
